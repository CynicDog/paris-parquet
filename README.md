...
**The column summaries cover every row, automatically.** The card above each column (nulls, range, mean, histogram, top values) describes the whole view, not the rows loaded: a slim status line says while the rest of the file is being counted, in the background, in batches that fit the memory budget, one row group at a time, and each card fills in when its batch is done. There is no button and no option; over a `WHERE` with more matches than were read the cards count the matches. A click on a histogram bar or a top value narrows the query to it and runs it over the whole file. Sorting a list (`ORDER BY`, or a click on a header) is likewise the top of the whole file, found by reading only the sort column, and a join streams the larger file a row group at a time and refuses, with its exact size, a result that would not fit.
